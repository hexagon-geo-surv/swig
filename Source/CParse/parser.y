...
%union {
  ...
  struct Define {
    ...
    // string or char type as a string (without any C/C++ syntax like quotes or
    // escaping).
    String *stringval;
    int     type;
...
cpp_vend       : cpp_const SEMI { 
                     Clear(scanner_ccode);
                     $$.val = 0;
                     $$.qualifier = $cpp_const.qualifier;
                     $$.refqualifier = $cpp_const.refqualifier;
                     $$.bitfield = 0;
...
edecl          :  identifier {
		   ...
		   if ($etype.stringval) {
		     Setattr($$,"enumrawvalue", $etype.stringval);
		   }
...
exprsimple     : exprnum
		  ...
		  $$.unary_arg_type = 0;
               ...
               | CHARCONST {
	       // Here and elsewhere a zero byte in the string value truncates rather than being escaped by %(escape)s FIXME FIXME
	          $$.stringval = $CHARCONST;
